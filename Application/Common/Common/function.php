...
function list_to_tree_menu($list, $pk = 'id', $pid = 'pid', $child = '_child', $root = 0)
{
    // 创建Tree
    $tree = array();
    if (is_array($list)) {
...
function getDoc($catid, $limit = 6)
{
    ...
    foreach ($ret as &$art) {
        ...
        $art['cover'] = getcover($art['cover_id']);
    }
    return $ret;
}
...
{
    //查询中间栏目1
    $ret = D('Category')->where(array('id' => array('in', $catid)))->order('sort asc')->limit($limit)->select();
    foreach ($ret as &$art) {
        $art['cover'] = getcover($art['icon']);
    }
    return $ret;
}

function getCatArrayDoc($catid, $limit = 6, $doclimit = 6)
{
    //查询中间栏目1
//    echo $catid;
    $ret = D('Category')->where(array('id' => array('in', $catid)))->order('sort asc')->limit($limit)->select();
    foreach ($ret as &$arts) {
        $arts['cover'] = getcover($arts['icon']);
        $arts['arts'] = D('Document')->where(array('category_id' => $arts['id'], 'status' => array('gt', 0)))->order('create_time desc')->limit($doclimit)->select();
        foreach ($arts['arts'] as &$art) {
            $art = D('Document')->detail($art['id']);
            if ($art['cover_id'] > 0) {
                $art['cover'] = getcover($art['cover_id']);
            } else {
                $art['cover'] = array('path' => 'Public/Home/images/xk/nopic.jpg');
            }
        }
    }
    return $ret;
...
function getcover($coverid){
    if ($coverid > 0) {
        return get_cover($coverid);
    } else {
        return array('path' => 'Public/Home/images/xk/nopic.jpg');
    }
}
function getCats($catid, $limit = 6)
{
    //查询中间栏目1
    $ret = D('Category')->where(array('pid' => array('in', $catid)))->order('sort asc')->limit($limit)->select();
    foreach ($ret as &$art) {
        $art['cover'] = getcover($art['icon']);
    }
    return $ret;
...
function getCatsAndDoc($catid, $limit = 6, $doclimit = 6)
{
    //查询中间栏目1
    $ret = D('Category')->where(array('pid' => array('in', $catid)))->order('sort asc')->limit($limit)->select();
    foreach ($ret as &$arts) {
...
function getHot($limit = 6)
{
    //查询中间栏目1
    $ret = D('Document')->where(array('status' => array('gt', 0)))->order('view desc')->limit($limit)->select();
    foreach ($ret as &$arts) {
